...
const Hombre = () => {
  ...
  const getColorCode = (color) => ({
    'Azul': '#4285f4', 'Azul Marino': '#3b78e7', 'Negro': '#2c2c54', 'Blanco': '#ffffff', 'Rojo': '#e74c3c',
    'Verde': '#2ecc71', 'Amarillo': '#f1c40f', 'Gris': '#7f8c8d', 'Morado': '#9b59b6', 'Rosa': '#e252b2',
    'Naranja': '#e67e22', 'Café': '#795548', 'Beige': '#f5f5dc'
  }[color] || '#cccccc');
  ...
  const handleProductClick = (product) => navigate(`/detallesp/${product.id}`);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} whileHover={{ y: -10 }}>
        <Paper elevation={2} sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: '16px', overflow: 'hidden', backgroundColor: customColors.cardBg, transition: 'all 0.3s ease', '&:hover': { boxShadow: `0 10px 20px ${alpha(customColors.primary, 0.15)}` }}}>
          <Box sx={{ position: 'relative' }}>
            ...
            <Chip label={colorName} size="small" sx={{ position: 'absolute', top: 12, right: 12, backgroundColor: colorCode, color: ['Blanco', 'Amarillo', 'Beige'].includes(colorName) ? '#000000' : '#ffffff', fontWeight: 'bold', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }} />
          </Box>
      ...
      <Stack direction="row" flexWrap="wrap" gap={1}>
        ...
        {colores.map(color => (
          <Chip
            ...
            avatar={<Box sx={{ width: 20, height: 20, borderRadius: '50%', backgroundColor: getColorCode(color.color), border: '1px solid rgba(0,0,0,0.1)' }} />}
            color={colorFilter === color.id ? 'primary' : 'default'}
      ...
      <Container maxWidth="xl" sx={{ py: 5 }}>
        ...
        <Box sx={{ mb: 3, display: 'flex', justifyContent: 'flex-start' }}>
          <TextField 
            ...
            size="small"
            sx={{ 
              width: { xs: '100%', sm: '250px' }, // Ajustado para coincidir con el ancho del filtro
