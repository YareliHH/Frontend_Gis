...
const Hombre = () => {
  ...
  const customColors = {
    ...
    accent: '#0288d1',         // Azul más oscuro para acentos
    lightAccent: '#81d4fa',    // Azul claro
    rose: '#4fc3f7',           // Azul claro para reemplazar el rosa
    background: isDarkMode ? '#121212' : '#e3f2fd', // Fondo azul claro en modo claro
    cardBg: isDarkMode ? '#1e1e2d' : '#ffffff',     // Fondo de las tarjetas
    textPrimary: isDarkMode ? '#ffffff' : '#0d47a1', // Texto principal azul oscuro
    textSecondary: isDarkMode ? '#b3b3cc' : '#1976d2', // Texto secundario azul medio
    gradient: 'linear-gradient(135deg, #1e88e5 0%, #4fc3f7 100%)' // Gradiente azul
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const [productosResult, categoriasResult, coloresResult, tallasResult] = await Promise.all([
        fetchData('https://backend-gis-1.onrender.com/api/Hombres'), // o 'https://backend-gis-1.onrender.com/api/Mujeres' para ese componente
